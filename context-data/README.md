...
repo as source evidence, asks the model-backed context planner when credentials
are available, and falls back to the deterministic planner with an explicit
`heuristic` mode when no model proposal can be used.
