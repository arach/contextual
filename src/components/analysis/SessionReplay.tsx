...
// Session replay — ONE session, turn by turn: header + accumulation mountain +
// transport + scrollable timeline + per-turn impact + footer.
//
// This is the shared, single-source instrument. It was extracted verbatim from
// the studio SessionObserve prototype so BOTH the studio route
// (`/studio/session-observe`) and the Explore workbench render the exact same
// instrument with no duplicated logic. The studio owns the session LIST + fetch
// shell; this file owns the instrument for ONE session.
//
// A REPORTING instrument. It reports impact; it never editorializes. A turn is
// never rated. There is no "noise", no "signal", no "waste" — only who spoke,
// what they did (a neutral gist), and how much it ADDED to the window.
//
// THREE ZONES, one shared cursor (= the current turn's row index), driven by a
// TRANSPORT (play / pause / prev / next) as well as by scroll and clicks:
//   1. FIXED HEADER — stat-bar (session meta) + transport, then the compact
//      ACCUMULATION mountain (cumulative window tokens by turn, stacked by
//      bucket, with a vertical playhead ◆ and the bucket legend).
//   2. SCROLLABLE TIMELINE — the ONLY thing that scrolls. A vertical
//      conversation log, one row per turnIndex: role bead, gist, +N delta,
//      bucket chips. Scrolling drives the playhead; clicking a row (or the
//      mountain) seeks; PLAY auto-advances one turn at a time (~700ms).
//   3. RIGHT COLUMN — "Impact of turn N": the turn's role · gist, its +N delta,
//      per-bucket deltas, and the window AS OF turn N. Collapses on narrower
//      embeds; the timeline then goes full width and the active row expands its
//      impact inline.
//
// RESPONSIVE — keyed off the instrument's OWN container width (a ResizeObserver
// on the root wrapper), NOT the viewport. Inside Explore the center column is
// narrower than the viewport (left rail + inspector eat width), so a viewport
// query would pick the wrong tier. Measuring the instrument itself keeps it
// correct full-width in the studio AND boxed inside Explore.
...
import { Bot, Pause, Play, SkipBack, SkipForward, User, Wrench } from "lucide-react";
...
/* ------------------------------------------------------------------ *
 * Input shape — the minimal session the instrument needs. Structurally a
 * subset of both the studio fetch payload and the Explore `SessionAnalysis`.
 * ------------------------------------------------------------------ */
...
/** Distinct turnIndex values carrying atoms — the instrument's turn count. */
export function distinctTurns(s: ObserveSession): number {
  const seen = new Set<number>();
  for (const a of s.atoms) seen.add(a.turnIndex ?? 0);
  return seen.size;
}
...
/* ------------------------------------------------------------------ *
 * Local palette — scoped to the instrument container (matches siblings).
 * Exported so the studio shell chrome (list / loading / error) shares the vars.
 * ------------------------------------------------------------------ */
export const OBSERVE_PALETTE = {
  "--hg-bg": "#0d1117",
  "--hg-bg-tint": "#161c24",
  "--hg-surface": "#1b232c",
  "--hg-surface-2": "#212b35",
...
/* ------------------------------------------------------------------ *
 * Responsive tier — keyed off the instrument's OWN container width via a
 * ResizeObserver on the root wrapper (NOT the viewport). Tiers, tuned to the
 * instrument's real content (the impact column is a fixed 360px, so "wide"
 * needs enough left for a comfortable timeline):
 *   • "wide"  ≥ 1000px : timeline + right impact column together.
 *   • "mid"   760–1000 : timeline full width; impact collapses inline.
 *   • "narrow" < 760px : timeline full width; impact collapses inline.
 * Only "wide" mounts the right impact column; "mid"/"narrow" surface the active
 * row's impact inline. SSR-safe: renders "wide" before the first measure. The
 * wide floor (1000px) leaves a comfortable ≥640px timeline beside the fixed
 * 360px impact column, with margin over Explore's ~1028px center at 1600px.
 * ------------------------------------------------------------------ */
type InstrumentTier = "narrow" | "mid" | "wide";
const TIER_MID_MIN_PX = 760;
const TIER_WIDE_MIN_PX = 1000;

function tierForWidth(width: number): InstrumentTier {
  if (width >= TIER_WIDE_MIN_PX) return "wide";
  if (width >= TIER_MID_MIN_PX) return "mid";
  return "narrow";
}

function useContainerTier(ref: RefObject<HTMLElement | null>): InstrumentTier {
  const [tier, setTier] = useState<InstrumentTier>("wide");
  useEffect(() => {
...
/* ================================================================== *
 * SessionReplay — the public component. Builds the model for ONE session and
 * renders the instrument body. Pass a stable `key` (e.g. session.id) at the
 * call site so switching sessions remounts and resets the cursor.
 * ================================================================== */
...
export function SessionReplay({ session }: SessionReplayProps) {
  const model = useMemo(() => buildSession(session), [session]);
  return <ObserveInstrumentBody model={model} />;
}

/* ================================================================== *
 * Instrument body — owns the cursor state machine and the scroll↔playhead
 * wiring, and measures its own width to pick the responsive tier.
 *
...
function ObserveInstrumentBody({ model }: { model: SessionModel }) {
  const rootRef = useRef<HTMLDivElement>(null);
  const tier = useContainerTier(rootRef);
  const lastRow = model.turns.length - 1;
  const showImpactColumn = tier === "wide"; // right column only on the wide tier
  ...
  return (
    ...
    >
      ...
      {/* ZONES 2 + 3 — scrollable timeline | responsive impact column.
          Gated by the CONTAINER tier (the instrument's own width). Only the
          "wide" tier mounts the right impact column; below that the timeline
          goes full width and the active row surfaces its impact inline. */}
      <div
        className="grid"
        style={{
 ...
 * that row's inline impact). Below the "wide" tier the right column is gone, so
 * the active row surfaces its impact inline instead.
 * ================================================================== */
...
/* ================================================================== *
 * ZONE 3 — Impact panel (right column). The impact of the CURRENT (cursor) turn
